...
| Incumbent adds equivalent routing | High | High | Vellum, Portkey, Cloudflare, a provider, or another gateway ships the required search policy and logs | audit the current stack first; configure or integrate rather than force replacement; own customer-specific outcome evidence | Reassess the wedge after every five qualified conversations and at least quarterly |
